...
public class InvitacionService {
 ...
 // Dentro de tu método crearInvitacion en InvitacionService.java, reemplaza el bloque del if (emailService != null):
if (emailService != null) {
    try {
        // Traducimos el texto del rol al ID numérico correspondiente para Thymeleaf
        int idCargoNum = switch (rol.toUpperCase()) {
            case "ADMIN" -> 1;
            case "DOCENTE" -> 2;
            case "ACUDIENTE" -> 3;
            case "ESTUDIANTE" -> 4;
        ...
        // Enviamos la invitación con los nuevos parámetros tracking
        emailService.enviarInvitacion(correo, guardada.getCodigo(), rol, idCargoNum, horasValidez);
    } catch (Exception e) {
        e.printStackTrace(); 
    }
}
